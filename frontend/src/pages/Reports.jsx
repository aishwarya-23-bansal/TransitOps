...
export default function Reports() {const [vehicles, setVehicles] = useState([])
...
const fuelEfficiency =
  trips.length > 0
    ? Math.round(
        (trips.filter((t) => t.fuelConsumed > 0).length / trips.length) * 100
      )
    : 0
...
const costControl =
  totalRevenue > 0
    ? Math.min(
        100,
        Math.round((completedTrips / Math.max(trips.length, 1)) * 100)
      )
    : 0
